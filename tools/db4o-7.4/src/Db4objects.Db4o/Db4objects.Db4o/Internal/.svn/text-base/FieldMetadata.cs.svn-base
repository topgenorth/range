	...
	public class FieldMetadata : ClassAspect, IStoredField
	{
		...
		public void TraverseValues(Transaction transaction, IVisitor4 userVisitor)
		{
			...
			lock (stream.Lock())
			{
				...
				_index.TraverseKeys(transaction, new _IVisitor4_952(this, userVisitor, context));
			}
		}

		private sealed class _IVisitor4_952 : IVisitor4
		{
			public _IVisitor4_952(FieldMetadata _enclosing, IVisitor4 userVisitor, IContext context
				)
			{
				this._enclosing = _enclosing;
		...
		public override void DefragAspect(IDefragmentContext context)
		{
			...
			context.SlotFormat().DoWithSlotIndirection(context, typeHandler, new _IClosure4_1113
				(context, typeHandler));
		}

		private sealed class _IClosure4_1113 : IClosure4
		{
			public _IClosure4_1113(IDefragmentContext context, ITypeHandler4 typeHandler)
			{
				this.context = context;
